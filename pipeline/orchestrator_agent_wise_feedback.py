...
# This filter is developed under the assistance of ChatGPT
...
# This function is guided by ChatGPT to remove the <think> tag generated by LLMs
...
def pipeline(user_query, max_retries=2):
    ...
    for iteration in range(max_retries + 1):
        ...
        def process_sub_task_with_feedback(sub_task_query):
            # only use subtask independently without original query
            query_for_retrieval = sub_task_query
            ...
            if task_feedback and iteration > 0:
                ...
                retrieval_result = brain_agent.retrieve(query_for_retrieval)
        ...
        # Enumerate contexts to make explicit references easier for the generation model
        enumerated_contexts = [
            f"Context {idx + 1}:\n{ctx}"
            for idx, ctx in enumerate(contexts_for_generation)
        ]
        ...
        if generation_feedback and iteration > 0:
            ...
            Your task:
            1. Answer the Question directly and concisely (≤150 words) using ONLY the information found in those context passages. When you use a passage, cite it like (Context N).
            2. Do NOT add external knowledge unless it is explicitly required by the FEEDBACK.
            3. If the answer cannot be found in the passages, reply exactly: CANNOT_FIND.
        ...
        # strip any chain-of-thought blocks like <think>...</think> or delineated markdown headers
        ...
        current_response_content = _clean_answer(generated_response)
        ...
        retrieval_avg = sum(e["score"] for e in retrieval_evals.values()) / len(
            retrieval_evals
        )
        ...
        # The following commented function is used to print out the feedback used in report
        # # Print detailed agent-wise feedback for this iteration
        # logger.info("Agent-wise feedback for this iteration:")
        # _log_multiline(
        #     "  Decomposition feedback: ", _clean_feedback(decomp_eval["feedback"])
        # )
        # for sub_task, eval_data in retrieval_evals.items():
        #     _log_multiline(
        #         f"  Retrieval feedback for '{sub_task}': ",
        #         _clean_feedback(eval_data["feedback"]),
        #     )
        # _log_multiline("  Generation feedback: ", _clean_feedback(gen_eval["feedback"]))
    ...
    return {
        ...
        "iteration_scores": iteration_logs,
    }


if __name__ == "__main__":
    # The following test is developed under the guidance of ChatGPT
