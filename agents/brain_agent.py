...
class BrainAgent:
    ...
    def __init__(self):
        ...
        # In case anyone read this and wondering if the following is generated by AI, it's NOT. It was linted by Black.
        self.tool_specs = [
            {
                "type": "function",
                "function": {
        ...
        if (
            ...
            try:
                tool_args_dict = json.loads(
                    tool_args_str
                )  # tool args is the task the tool is trying to solve
            ...
            if (
                ...
                # automatic fallback to web search when vector search yields nothing
                if tool_name == "vector_search":
                    logger.info(
                        "Vector search returned no results – falling back to web_search"
                    )
                    web_result = self.execute_tool("web_search", {"query": query})
                    tool_output_content = web_result
    ...
    # this function is called during the feedback loop to decide which tool it should use next
    ...
    # dynamically add improvements to original prompt
