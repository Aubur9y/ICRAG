...
if res:
    ...
    output_folder = "test_results/bloom/qwen8b"
    os.makedirs(output_folder, exist_ok=True)

    # save overall RAGAS score txt
    output_path = os.path.join(output_folder, f"level_ragas_scores_{timestamp}.txt")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(str(ragas_results))
    logger.info(f"Ragas scores saved to {output_path}")

    # save per-question QA pairs and generated answers as JSON
    qa_output_path = os.path.join(output_folder, f"qa_outputs_{timestamp}.json")
